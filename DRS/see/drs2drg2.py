		...
		if n.type == "cond":
			for sn in n.expression:
				if sn.type == "rel" or sn.type == "eq": # rel
					...
					#	continue
					#if (a1 not in declared_variable) and (a1 in projections):
					if a1 not in declared_variable:
						Tuples.append(" ".join([projections[a1], "REF", a1]))
						declared_variable.append(a1)
					#if (a2 not in declared_variable) and (a2 in projections):
...
if __name__ == "__main__":
	...
	for line in open(sys.argv[1]):
		line = line.strip()
		target = json.loads(line, object_hook=ascii_encode_dict)
