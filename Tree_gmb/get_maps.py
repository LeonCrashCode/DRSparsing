...
def build_arg_parser():
	...
	parser.add_argument('-align', required=True, type=str,
						help='aligns')
...
def get_matching_clauses(arg_list):
	...
	#print prod_drs.vars_seen
	#print gold_drs.vars_seen
	align = []
	"""
	mappings = []
	match_num = []
	for line in open(args.align):
		...
		if line.startswith("mappings"):
			mappings.append(line.split("[")[1][:-1].split(","))
		elif line.startswith("match_num:"):
			match_num.append(line.split()[-1])
	assert len(mappings) == len(match_num)
	
	max_match_num_idx = -1
	max_match_num = -1
	for i, num in enumerate(match_num):
		if num > max_match_num:
			max_match_num_idx = i
	assert max_match_num_idx != -1
	"""
	for line in open(args.align):
		line = line.strip()
		if line.startswith("best_mapping:"):
			align = line.split("[")[1][:-1].split(",")
	assert len(align) != 0

	for f,t in enumerate(align):
		t = int(t)
		if t == -1:
			print prod_drs.vars_seen[f],-1
		else:
			print prod_drs.vars_seen[f], gold_drs.vars_seen[t]
	exit(1)
