...
def rule_out_percent(toks):
    ...
    for tok in toks:
        ...
        ts.append(tok)
    return ts
def file_to_clfs2(filepath, v=0):
    '''reads a file with clausal forms inside it and
       returns a list of clausal forms where each form is a list of clauses/tuples
       An empty line is a separator of clausal forms
    '''
    ...
    with codecs.open(filepath, 'r', encoding='UTF-8') as f:
        for line in f:
            ...
            if line == "":
                ...
                i = idx + 1
                while i < len(lines):
                    lines[i] = rule_out_percent(lines[i].split())
                    if len(lines[i]) > 0:
                        list_of_clf[-1].append(tuple(lines[i]))
                    i += 1
                lines = []
            else:
                if line[0] == "#":
                    list_of_fil.append(line)
                    continue
                lines.append(line)
    return (list_of_clf, list_of_fil) 
...
if __name__ == '__main__':
    ...
    (clfs, raws) = file_to_clfs2(args.src, v=args.v) 
    total = len(clfs)
