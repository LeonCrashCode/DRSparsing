...
def add_candidate_mapping(candidate_mapping, weight_dict, weight_score, node1_index_drs1, node1_index_drs2, node2_index_drs1, node2_index_drs2, no_match, gold_clause_idx, prod_clause_idx):
	...
	candidate_mapping[node1_index_drs1].add(node1_index_drs2)
	candidate_mapping[node2_index_drs1].add(node2_index_drs2)
	#print node1_index_drs1, node1_index_drs2
	#print node2_index_drs1, node2_index_drs2
...
def map_two_vars_edges(clauses_prod, clauses_gold, prod_drs, gold_drs, edge_numbers, candidate_mapping, weight_dict, var_index1, var_index2, add_to_index, add_to_index2):
	...
	for i in range(0, len(clauses_prod)):
		for j in range(0, len(clauses_gold)):
			...
			#print clauses_prod[i]
			#print clauses_gold[j]
			...
			if not args.partial:
				...
				if allowed_var1 and allowed_var2 and allowed_edge and normalize(clauses_prod[i][edge_numbers[0]]) == normalize(clauses_gold[j][edge_numbers[0]]):
					...
					#print clauses_prod[i], clauses_gold[j]
...
def get_matching_clauses(arg_list):
	...
	#print prod_drs.op_two_vars
	#print prod_drs.op_two_vars_idx
	#print prod_drs.op_two_vars_abs
	#print prod_drs.op_two_vars_abs_idx
	#print prod_drs.op_three_vars
	#print prod_drs.op_three_vars_idx
	#print gold_drs.op_three_vars

	#print prod_drs.roles_abs
	#print prod_drs.roles_abs_idx
	#print prod_drs.roles
	#print prod_drs.roles_idx
	#print prod_drs.concepts
	#print prod_drs.concepts_idx

	#print prod_drs.type_vars
	#print prod_drs.var_map
	#print "=========="
	if single and (args.max_clauses > 0 and ((prod_drs.total_clauses > args.max_clauses) or (gold_drs.total_clauses > args.max_clauses))):
		...
		#print prod_drs.total_clauses, gold_drs.total_clauses
		#recompute best match num for EQU
		
		op_two_vars = []
		for i in range(len(prod_drs.op_two_vars)):
			op_two_vars.append(("b"+str(best_mapping[int(prod_drs.op_two_vars[i][0][1:])]), prod_drs.op_two_vars[i][1], "b"+str(best_mapping[int(prod_drs.op_two_vars[i][2][1:])])))
		op_two_vars_abs = []
		for i in range(len(prod_drs.op_two_vars_abs)):
			op_two_vars_abs.append(("b"+str(best_mapping[int(prod_drs.op_two_vars_abs[i][0][1:])]), prod_drs.op_two_vars_abs[i][1], "b"+str(best_mapping[int(prod_drs.op_two_vars_abs[i][2][1:])]), prod_drs.op_two_vars_abs[i][3]))
		op_three_vars = []
		for i in range(len(prod_drs.op_three_vars)):
			op_three_vars.append(("b"+str(best_mapping[int(prod_drs.op_three_vars[i][0][1:])]), prod_drs.op_three_vars[i][1], "b"+str(best_mapping[int(prod_drs.op_three_vars[i][2][1:])]), "b"+str(best_mapping[int(prod_drs.op_three_vars[i][3][1:])])))
		
		new_best_match_num = 0
		key = {}
		for i in range(len(op_two_vars)):
			if op_two_vars[i] in key:
				continue
			key.setdefault(op_two_vars[i])
			find = 0
			for j in range(len(gold_drs.op_two_vars)):
				if op_two_vars[i] == gold_drs.op_two_vars[j]:
					find = 1
					break
			if find:
				new_best_match_num += 1

		key = {}
		for i in range(len(op_two_vars_abs)):
			if op_two_vars_abs[i] in key:
				continue
			key.setdefault(op_two_vars_abs[i])
			find = 0
			for j in range(len(gold_drs.op_two_vars_abs)):
				if op_two_vars_abs[i] == gold_drs.op_two_vars_abs[j]:
					find = 1
					break
			if find:
				new_best_match_num += 1
		...
		key = {}
		for i in range(len(op_three_vars)):
			if op_three_vars[i] in key:
				continue
			key.setdefault(op_three_vars[i])
			find = 0
			for j in range(len(gold_drs.op_three_vars)):
				if op_three_vars[i][1] == "EQU":
					#print op_three_vars[i]
					if op_three_vars[i][:2] == gold_drs.op_three_vars[j][:2]:
						if (op_three_vars[i][2:] == gold_drs.op_three_vars[j][2:]) or (op_three_vars[i][2:] == gold_drs.op_three_vars[j][2:][::-1]):
							find = 1
							break
				else:
					if op_three_vars[i] == gold_drs.op_three_vars[j]:
						find = 1
						break
			if find:
				new_best_match_num += 1
		#print new_best_match_num
		...
		return [new_best_match_num, prod_drs.total_clauses, gold_drs.total_clauses, smart_fscores, found_idx, match_division, prod_clause_division, gold_clause_division, len(prod_drs.var_map)]
